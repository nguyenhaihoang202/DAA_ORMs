...
# ---- plot (pattern = correctness; fixed fill) ----
p_hmp_dir <- ggplot(
  df_counts,
  aes(x = method, y = n,
      group = correctness,
  ...
  ggpattern::geom_col_pattern(
    ...
    fill = "#F7B500",                 # <-- single neutral fill (no color by method)
  ...
  # (no scale_fill_manual since fill isn't mapped)
  ...
  guides(
    pattern = guide_legend(
      ...
      override.aes = list(fill = "#F7B500", colour = "black", linewidth = 0.4, group = NA)
    )
  ) +
