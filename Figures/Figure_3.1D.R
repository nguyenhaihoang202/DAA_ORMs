...
# ----------------------- File discovery --------------------------
# Bloomer scenario files

merged_files <- Sys.glob("bloomer_ADD_ALL_N*_M*_seed*.RData") %>% sort()
base_files   <- Sys.glob("bloomer_N*_M*_seed*.RData")         %>% sort()
...
if (!"scenario" %in% names(res_all)) {
  res_all <- res_all %>% mutate(scenario = "Bloomer")
}
...
SCEN <- "Bloomer"
res_bloomer <- res_all %>% filter(scenario == SCEN)

perf_bloomer <- res_bloomer %>%
...
p_fdr <- ggplot(perf_bloomer, aes(x = fdr, y = method, fill = method, color = method)) +
...
p_pow <- ggplot(perf_bloomer, aes(x = power, y = method, fill = method, color = method)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.28, width = 0.6, linewidth = 0.35) +
  geom_jitter(height = 0.16, width = 0, size = 1.1, alpha = 0.85) +
  scale_fill_manual(values = method_colors, drop = FALSE) +
  scale_color_manual(values = method_colors, drop = FALSE) +
...
p_title <- ggplot() +
  annotate("text", x = 0, y = 0, label = "Bloomer Scenario",
...
ggsave("figures/fig4_bloomer_combined.png", combined, width = 10, height = 5, dpi = 300)
message("Saved: figures/fig4_bloomer_combined.png")
