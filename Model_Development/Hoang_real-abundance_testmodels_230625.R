...
results_imp <- map2_dfr(
  ...
  .f = function(y, K) {
    ...
    draws_imp <- fit_imp$draws(variables = c("beta_0", "beta_1")) |> as_draws_df()
    diff_imp <- draws_imp$beta_1 - draws_imp$beta_0
    
    tibble(
      est = median(diff_imp),
      lwr95 = quantile(diff_imp, 0.025),
      upr95 = quantile(diff_imp, 0.975),
      p_lt_0 = mean(diff_imp < 0),
      p_gt_0 = mean(diff_imp > 0)
    )
  }
)
