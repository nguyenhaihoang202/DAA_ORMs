...
normalize_cols <- function(df) {
  ...
  # estimate <- prefer 'estimate'; else 'est' or 'beta'; otherwise NA
  if (!"estimate" %in% nm) {
    if ("est" %in% nm) {
...
# ---------- find files (FORCED tags) ----------
base_files  <- Sys.glob("sym_FORCED_N*_M*_seed*.RData") %>% sort()
extra_files <- Sys.glob("sym_FORCED_EXTRAONLY_SUP_N*_M*_seed*.RData") %>% sort()
...
for (sd in seeds_to_merge) {
  ...
  RUN_TAG <- sprintf("sym_FORCED_ADD_ALL_N%d_M%d_seed%03d", n, M_base, SEED_MAIN)
