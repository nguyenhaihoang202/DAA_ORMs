...
run_corncob <- function(counts, meta, fm = ~ group, ev = TRUE,
  ...
  # Choose a single p-value per taxon
  if (stat == "Wald") {
    out <- coef_tab %>%
    ...
    # Use LRT p-values from differentialTest; keep est/se for direction/magnitude
    out <- coef_tab %>%
...
# ---------------- biased simulator  ----------------
simulate_counts_once_biased <- function(seed, n, mean_vars, cors) {
  set.seed(seed)
  ...
  # Biased effects: 30% nonzero, mean +1 on latent normal scale
  non_zero_effect <- rbinom(n_taxa, size = 1, prob = 0.30)
  effects <- rnorm(n_taxa, mean = 1, sd = 0.5) * non_zero_effect
  names(effects) <- paste0("taxon_", seq_len(n_taxa))
  
  # Add effects to cases
  ...
  # JP: This was wrong! Those gamma parameters are taxon-wise, that is, each taxon
  # has certain gamma distribution (for all samples)!
  # I changed byrow = TRUE -> byrow = FALSE
  ...
  # Taxon-wise detection bias, closure, library sizes, Poisson sampling
...
for (SEED_MAIN in SEEDS) {
  message(">>> Bias-from-scratch extras for SEED = ", SEED_MAIN)
  sim <- simulate_counts_once_biased(SEED_MAIN, n, mean_vars, cors)
  
  meta <- data.frame(
    group   = factor(sim$groups, labels = c("control", "case")),
    data_id = sprintf("bias_SUP_seed%03d", SEED_MAIN),
  ...
  RUN_TAG <- sprintf("bias_EXTRAONLY_SUP_N%d_M%d_seed%03d", n, ncol(sim$counts2), SEED_MAIN)
  save(
    SEED_MAIN, n,
