# ============================================================
# Extra methods for structural-zero (prevalence-only) scenario
# Baseline files save as: prevONLY_N*_M*_seed*.RData
# This script produces:   prevONLY_EXTRAONLY_SUP_N*_M*_seed*.RData
# Methods added: DESeq2, MaAsLin2, corncob, LDM
# Ground truth = prevalence effect (beta_prev), not abundance shift
# ============================================================
...
# ---------------- prevalence-only simulator ----------------
simulate_counts_prevONLY <- function(seed, n, mean_vars, cors) {
  set.seed(seed)
  n_taxa <- nrow(mean_vars)
  groups <- rep(0:1, each = n / 2)
  
  # Abundance quantiles (no group effect on abundance among positives)
  mvn_latent <- MASS::mvrnorm(n, mu = rep(0, n_taxa), Sigma = cors)
  
  # Baseline prevalence & prevalence effects (logit scale)
  alpha_prev <- rnorm(n_taxa, mean = -2, sd = 1)  # ~10–20% baseline presence
  ...
  beta_prev[prev_nz_idx] <- rnorm(nnz_prev, mean = 0.8, sd = 0.3)  # OR ≈ 2.2 up in cases
  
  # Presence/absence matrix
  beta_mat  <- (cbind(replicate(n, beta_prev)) |> t()) * groups
  alpha_mat <- matrix(rep(alpha_prev, each = n), nrow = n, byrow = FALSE)
  p_prev    <- plogis(alpha_mat + beta_mat)
  A <- matrix(rbinom(n * n_taxa, size = 1, prob = as.vector(p_prev)),
              nrow = n, ncol = n_taxa)
  
  # Gamma marginals for abundance among positives (taxon-wise parameters)
  ...
  true_abundances  <- abund_if_present * A   # structural zeros
  
  # Detection biases, closure, library sizes, Poisson sampling
  ...
  colnames(counts) <- paste0("taxon_", seq_len(n_taxa))
  
  # Prevalence filter and outputs
  ...
  list(
    ...
    groups        = groups,
    true_effects  = tibble::tibble(taxon = paste0("taxon_", seq_len(n_taxa)),
...
for (SEED_MAIN in SEEDS) {
  message(">>> Prevalence-only extras for SEED = ", SEED_MAIN)
  sim <- simulate_counts_prevONLY(SEED_MAIN, n, mean_vars, cors)
  
  # Metadata
  meta <- data.frame(
    group   = factor(sim$groups, labels = c("control", "case")),
    data_id = sprintf("prev_SUP_seed%03d", SEED_MAIN),
  ...
  # Add truth + flags (truth = prevalence effect)
  ...
  RUN_TAG <- sprintf("prevONLY_EXTRAONLY_SUP_N%d_M%d_seed%03d", n, ncol(sim$counts2), SEED_MAIN)
  save(
    SEED_MAIN, n,
