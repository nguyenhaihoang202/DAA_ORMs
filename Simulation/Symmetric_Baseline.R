...
for (SEED_MAIN in 25:34) {
  set.seed(SEED_MAIN)
  message(">>> Running replicate with SEED_MAIN = ", SEED_MAIN)

  # ---------------- Symmetric scenario (forced balanced signs) ----------------
  # Latent MVN
  mvn_latent <- MASS::mvrnorm(n, mu = rep(0, n_taxa), Sigma = cors)
  
  # Balanced effects with many exact zeros (≈30% nonzero)  [FORCED BALANCE]
  prop_nonzero <- 0.30
  nnz   <- max(1L, round(prop_nonzero * n_taxa))
  nz_idx <- sample.int(n_taxa, nnz, replace = FALSE)
  
  # draw magnitudes; enforce exactly half positive / half negative
  mags  <- abs(rnorm(nnz, mean = 0, sd = 0.5))
  kpos  <- floor(nnz / 2)
  pos_j <- sample(nz_idx, kpos, replace = FALSE)
  neg_j <- setdiff(nz_idx, pos_j)
  
  effects <- numeric(n_taxa)
  effects[pos_j] <-  mags[seq_len(kpos)]
  effects[neg_j] <- -mags[(kpos + 1):nnz]
  
  # optional but recommended: center nonzeros to remove tiny residual mean
  effects[nz_idx] <- effects[nz_idx] - mean(effects[nz_idx])
  ...
  # These gamma parameters are taxon-wise: each taxon keeps the same shape/scale across samples
  ...
  # Taxon-wise biases, closure, Poisson sampling
  ...
  run_orm <- function(abundance, metadata, formula) {
    ...
    tibble::rownames_to_column(res_freq, "variable")
  }
  ...
  # ---- ORM  ----
  res_orm2 <- res_orm |> left_join(true_effects, by="taxon") |>
  ...
  invisible(
    res_all |>
      group_by(method) |>
      summarize(tp = sum(true_positive),
  ...
  # ---------------- QC diagnostics to save ----------------
  # NEW: sign skew among true effects and pre-closure total-load ratio
  nnz  <- sum(effects != 0)                                # NEW
  npos <- sum(effects > 0)                                 # NEW
  sign_skew <- if (nnz == 0) 0 else abs(npos - (nnz - npos)) / nnz   # NEW
  
  tot_case <- rowSums(true_abundances[groups == 1, , drop = FALSE])  # NEW
  tot_ctrl <- rowSums(true_abundances[groups == 0, , drop = FALSE])  # NEW
  load_ratio_med <- median(tot_case / tot_ctrl)                       # NEW
  
  # ---------------- Combine and save ----------------
  res_all2 <- bind_rows(res_all, res_bziolr, res_balor) |>
    mutate(taxon = fct_reorder(taxon, -estimate, .na_rm = TRUE))
  
  RUN_TAG <- sprintf("sym_FORCED_N%d_M%d_seed%03d", n, ncol(counts2), SEED_MAIN)  # CHANGED tag
  save(
    ...
    # QC
    effects, nz_idx, sign_skew, load_ratio_med,                          # NEW saved
