...
SEED_MAIN = 55
  # ---------------- Bias case ----------------
  # 1) Latent MVN with anchored cross-taxon correlations
  mvn_latent <- MASS::mvrnorm(n, mu = rep(0, n_taxa), Sigma = cors)
  
  # 2) Group effects: 30% nonzero, mean +1 on latent normal scale (bias case)
  #    This induces a net positive shift (compositional pressure after closure)
  ...
  # Apply effects to case group on latent normals
  effects_matrix <- (cbind(replicate(n, effects)) |> t()) * groups
  mvn_latent_plus_effect <- mvn_latent + effects_matrix
  
  # 3) Map to absolute abundances via gamma marginals (Vandeputte params)
  ...
  # FIX: columns constant across samples (per taxon)
  ...
  true_abundances <- qgamma(mvu_latent, shape = m_gamma_shapes, scale = m_gamma_scales)
  
  
  # 4) Taxon-wise detection biases (measurement bias), closure, and counts
  ...
  # ---- ANCOM-BC (alpha=0.10, 90% CI for plotting) ----
  ...
  RUN_TAG <- sprintf("bias_N%d_M%d_seed%03d", n, ncol(counts2), SEED_MAIN)
  save(
