# ============================================================
# add_methods_forced_sym.R
# Extras (DESeq2 / MaAsLin2 / corncob / LDM) for:
# Symmetric scenario with FORCED balanced signs (~30% nonzeros)
# ============================================================
...
run_corncob <- function(counts, meta, fm = ~ group, ev = TRUE,
  ...
  obj <- corncob::differentialTest(
    ...
    test              = "LRT"           # fits once; we can still make Wald p from coef
  )
  
  # Pull coef/SE from each fitted model for the group effect (2nd row)
...
# ---------------- symmetric FORCED simulator ----------------
simulate_counts_once_forced <- function(seed, n, mean_vars, cors) {
  set.seed(seed)
  ...
  # Forced balanced signs with many exact zeros (≈30% nonzero)
  prop_nonzero <- 0.30
  ...
  # Observation: taxon-wise biases, closure, Poisson sampling
  ...
  # QC
  nnz_all  <- sum(effects != 0)
  npos_all <- sum(effects > 0)
  sign_skew <- if (nnz_all == 0) 0 else abs(npos_all - (nnz_all - npos_all)) / nnz_all
...
for (SEED_MAIN in SEEDS) {
  message(">>> Symmetric-FORCED extras for SEED = ", SEED_MAIN)
  sim <- simulate_counts_once_forced(SEED_MAIN, n, mean_vars, cors)
  
  meta <- data.frame(
    group   = factor(sim$groups, labels = c("control", "case")),
    data_id = sprintf("sym_FORCED_SUP_seed%03d", SEED_MAIN),
  ...
  effects        <- sim$effects
  nz_idx         <- sim$nz_idx
  sign_skew      <- sim$sign_skew
  load_ratio_med <- sim$load_ratio_med
  
  RUN_TAG <- sprintf("sym_FORCED_EXTRAONLY_SUP_N%d_M%d_seed%03d", n, ncol(sim$counts2), SEED_MAIN)
  save(
    SEED_MAIN, n,
    ...
    res_extras_only,
    # QC
    effects, nz_idx, sign_skew, load_ratio_med,
