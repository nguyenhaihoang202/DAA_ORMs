...
normalize_cols <- function(df) {
  ...
  # estimate: use 'estimate' if present; else derive from 'est'/'beta'; else NA
  if (!"estimate" %in% nm) {
    if ("est" %in% nm) {
...
extra_files <- Sys.glob("realistic_EXTRAONLY_SUP_N*_M*_seed*.RData") %>% sort()
...
for (sd in seeds_to_merge) {
  ...
  RUN_TAG <- sprintf("realistic_ADD_ALL_N%d_M%d_seed%03d", n, M_base, SEED_MAIN)
