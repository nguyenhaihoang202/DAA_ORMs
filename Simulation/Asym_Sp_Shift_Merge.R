...
normalize_cols <- function(df) {
  ...
  # --- estimate: prefer 'estimate'; else copy from 'est' if present; else create NA
  if (!"estimate" %in% nm) {
    if ("est" %in% nm) {
      df <- df %>%
        mutate(estimate = .data$est) %>%
        select(-est)
    } else if ("beta" %in% nm) {
  ...
  # --- significant: prefer q; else use CI sign if available; else NA
...
# ---------- find files (BIAS case) ----------
base_files  <- Sys.glob("bias_N*_M*_seed*.RData") %>% sort()
extra_files <- Sys.glob("bias_EXTRAONLY_SUP_N*_M*_seed*.RData") %>% sort()
...
for (sd in seeds_to_merge) {
  ...
  base_df   <- as_tibble(e_base$res_all2)         %>% mutate(seed = e_base$SEED_MAIN)
  extras_df <- as_tibble(e_extra$res_extras_only) %>% mutate(seed = e_extra$SEED_MAIN)
  ...
  # carry useful objects from baseline (include bias-specific items if present)
  ...
  rel_abundances<- if (exists("rel_abundances", envir = e_base)) e_base$rel_abundances else NULL
  K_ord     <- if (exists("K_ord",         envir = e_base)) e_base$K_ord         else NULL
  stan_data <- if (exists("stan_data",     envir = e_base)) e_base$stan_data     else NULL
  
  M_base <- if (!is.null(counts2)) ncol(counts2) else NA_integer_
  
  RUN_TAG <- sprintf("bias_ADD_ALL_N%d_M%d_seed%03d", n, M_base, SEED_MAIN)
  
  save(SEED_MAIN, n, counts, counts2, groups, true_effects,
       rel_abundances, K_ord, stan_data,
       res_merged,
       file = paste0(RUN_TAG, ".RData"))
  
  cat(sprintf("Saved: %s.[RData]  rows=%d  methods=%s\n",
