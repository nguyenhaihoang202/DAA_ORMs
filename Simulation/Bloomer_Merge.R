...
normalize_cols <- function(df) {
  ...
  # significant: prefer q; else CI sign; else NA
...
extra_files <- Sys.glob("bloomer_EXTRAONLY_SUP_N*_M*_seed*.RData") %>% sort()
...
for (sd in seeds_to_merge) {
  ...
  RUN_TAG <- sprintf("bloomer_ADD_ALL_N%d_M%d_seed%03d", n, M_base, SEED_MAIN)
