...
# ---------------- bloomer simulator ----------------
simulate_counts_bloomer <- function(seed, n, mean_vars, cors) {
  set.seed(seed)
  ...
  # One strong bloomer, others zero
...
for (SEED_MAIN in SEEDS) {
  message(">>> Bloomer extras for SEED = ", SEED_MAIN)
  sim <- simulate_counts_bloomer(SEED_MAIN, n, mean_vars, cors)
  
  # Metadata
  meta <- data.frame(
    group   = factor(sim$groups, labels = c("control", "case")),
    data_id = sprintf("bloomer_SUP_seed%03d", SEED_MAIN),
  ...
  RUN_TAG <- sprintf("bloomer_EXTRAONLY_SUP_N%d_M%d_seed%03d", n, ncol(sim$counts2), SEED_MAIN)
  save(
    SEED_MAIN, n,
