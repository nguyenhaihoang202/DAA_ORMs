...
normalize_cols <- function(df) {
  ...
  # lwr/upr from estimate ± Z90*se if missing
  if (!"lwr" %in% names(df)) {
  ...
  # significant: prefer q; else CI exclusion; else NA
  ...
  # types + order (keep any extra cols like 'seed', 'data_id')
...
# ---------- find files (PREVALENCE-ONLY) ----------
base_files  <- Sys.glob("prevONLY_N*_M*_seed*.RData") %>% sort()
extra_files <- Sys.glob("prevONLY_EXTRAONLY_SUP_N*_M*_seed*.RData") %>% sort()
...
for (sd in seeds_to_merge) {
  ...
  RUN_TAG <- sprintf("prevONLY_ADD_ALL_N%d_M%d_seed%03d", n, M_base, SEED_MAIN)
