...
run_corncob <- function(counts, meta, fm = ~ group, ev = TRUE, stat = c("LRT","Wald")) {
  ...
  # Pull coef/SE for group effect from each fitted model
  ...
  coef_tab <- dplyr::bind_rows(res_list) %>%
    ...
    dplyr::mutate(taxon = rownames(t(counts)), df = Inf)
...
run_ldm <- function(counts, meta, fm = ~ group, clr = FALSE){
  ...
  obj <- LDM::ldm(formula = fmla, data = meta, comp.anal = clr, verbose = FALSE, n.cores = 1)
...
# ---------------- REALISTIC simulator (matches your base generator) ----------------
simulate_counts_once_realistic <- function(seed, n, mean_vars, cors) {
  ...
  # Filters (match base: rel_abund by rel>0; counts2 by counts>0)
...
for (SEED_MAIN in SEEDS) {
  ...
  # Add truth + flags (note: realistic has no exact zeros in truth)
  ...
  # Export objects for strict merge later
  counts         <- sim$counts
  counts2        <- sim$counts2
  ...
  RUN_TAG <- sprintf("realistic_EXTRAONLY_SUP_N%d_M%d_seed%03d", n, ncol(sim$counts2), SEED_MAIN)
  save(
    SEED_MAIN, n,
