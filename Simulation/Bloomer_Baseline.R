...
for (SEED_MAIN in 25:34) {
  ...
  # One strong bloomer on latent normal scale, others zero
  effects <- rep(0, n_taxa)
  effects[1] <- 3
  names(effects) <- paste0("taxon_", 1:n_taxa)
  
  # Add effects to latent variables for the case group
  ...
  # Taxon-wise detection biases, closure, Poisson sampling
  ...
  RUN_TAG <- sprintf("bloomer_N%d_M%d_seed%03d", n, ncol(counts2), SEED_MAIN)
  save(
