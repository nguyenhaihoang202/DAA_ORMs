...
mod_bayes <- cmdstan_model("BALOR.stan", cpp_options = list(stan_threads = TRUE))
...
# Compile + sample
mod_zi <- cmdstan_model("BZIOLR.stan",
...
# -----------------------------
# Build/repair BALOR and bziolr calls (no refits)
...
make_calls_generic <- function(res_df, bvs, pos = "supragingival", neg = "subgingival",
  ...
  if (all(c("p_gt0","p_lt0") %in% names(x))) {
    ...
    stop("Need p_gt0/p_lt0 or CI columns (lwr/upr or lwr95/upr95) to call BALOR/bziolr.")
  }
  x
}

if (!exists("res_balor")) stop("res_balor not found. Load your saved BALOR results.")
if (!exists("res_bziolr")) stop("res_bziolr not found. Load your saved bziolr results.")
...
if (!exists("res_bziolr_calls") || !"call" %in% names(res_bziolr_calls)) {
  res_bziolr_calls <- make_calls_generic(res_bziolr, bvs, "supragingival", "subgingival", PP_THRESH)
}
...
# No refits. Reuses res, res_ancombc, res_linda, res_balor, res_bziolr.
# ============================================================
