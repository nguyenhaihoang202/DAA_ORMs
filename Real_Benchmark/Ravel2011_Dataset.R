...
mod_bayes4 <- cmdstan_model("BALOR.stan", cpp_options = list(stan_threads = TRUE))
...
mod_zi <- cmdstan_model("BZIOLR.stan", cpp_options = list(stan_threads = TRUE))
...
eval_zi     <- eval_zi     %>% dplyr::mutate(model = "bziolr")
...
rename_obj("res_zi",           "res_bziolr")
rename_obj("res_zi_calls",     "res_bziolr_calls")
rename_obj("eval_zi",          "eval_bziolr")
...
rename_obj("fit_zi",           "fit_bziolr")
rename_obj("stan_data_zi",     "stan_data_bziolr")
...
if (exists("side_by_side")) {
  if ("model" %in% names(side_by_side)) {
    ...
    needed <- c("eval_orm","eval_ancombc","eval_linda","eval_balor","eval_bziolr")
    if (all(vapply(needed, exists, logical(1)))) {
      side_by_side <- dplyr::bind_rows(
        eval_orm, eval_ancombc, eval_linda, eval_balor, eval_bziolr
      ) %>% dplyr::select(model, TP, FP, FN, precision, recall)
    }
  }
}
...
# No refits. Reuses res, res_ancombc, res_linda, res_bayes4/res_zi (→ balor/bziolr)
# ======================================================================
...
# --- ensure BALOR / bziolr names (no refit) ---
rename_obj("res_bayes4", "res_balor")
rename_obj("res_zi",     "res_bziolr")
...
# --- Bayesian evaluator (BALOR, bziolr): PP_THRESH = 1 - alpha/2 ---
...
# BALOR/bziolr: ensure present and join to bvs
if (!exists("res_balor")) stop("res_balor not found. Use the rename above or load it.")
if (!exists("res_bziolr")) stop("res_bziolr not found. Use the rename above or load it.")
df_balor   <- res_balor %>% dplyr::left_join(bvs, by = "taxon")
df_bziolr   <- res_bziolr %>% dplyr::left_join(bvs, by = "taxon")
...
side_by_side_grid <- dplyr::bind_rows(
  grid_orm, grid_ancombc, grid_linda, grid_balor, grid_bziolr
